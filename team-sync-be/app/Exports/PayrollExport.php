...
class PayrollExport implements FromCollection, ShouldAutoSize, WithEvents, WithHeadings, WithMapping, WithStyles, WithTitle
{
    ...
    public function __construct($payrollId)
    {
        ...
        $this->fallbackWorkingDays = $this->resolveFallbackWorkingDays();
    }
    ...
    private function resolveWorkingDaysForDetail(PayrollDetail $detail): int
    {
        $effectiveWorkingDays = (int) ($detail->effective_working_days ?? 0);

        if ($effectiveWorkingDays > 0) {
            return $effectiveWorkingDays;
        }

        return $this->fallbackWorkingDays;
    }

    private function resolveFallbackWorkingDays(): int
    {
        $version = $this->payroll->payrollSettingVersion;

        if (! $version) {
            return 22;
        }

        if ($version->working_days_mode === 'fixed' && (int) $version->default_working_days > 0) {
            return (int) $version->default_working_days;
        }
