...
class AnalyticsRepository implements AnalyticsRepositoryInterface
{
    public function getExecutiveSummary(string $period, ?string $department, ?int $teamId): array
    {
        ...
        return cache()->remember($cacheKey, CacheConstants::ONE_HOUR, function () use ($parsed, $department, $teamId) {
            ...
            $employeeKpis = $this->getEmployeeKpis($filteredEmployeeIds, $start, $prevStart);
            $attendanceKpis = $this->getAttendanceKpis($filteredEmployeeIds, $start, $end, $prevStart, $prevEnd);
            $salaryKpis = $this->getSalaryKpis($filteredEmployeeIds, $start, $end, $prevStart, $prevEnd);
            $projectKpis = $this->getProjectKpis($filteredEmployeeIds);
            $leaveKpis = $this->getLeaveKpis($filteredEmployeeIds, $employeeKpis['total_employees'], $start, $end);
            $trendData = $this->getTrendData($filteredEmployeeIds, $start, $end);
            $teamPerformance = $this->getTeamPerformanceData($start, $end, $department, $teamId);
            ...
            return [
                ...
                'kpis' => [
                    'total_employees' => $employeeKpis['total_employees'],
                    'employee_growth' => $employeeKpis['employee_change'],
                    'attendance_rate' => $attendanceKpis['attendance_rate'],
                    'attendance_rate_change' => $attendanceKpis['attendance_change'],
                    'average_salary' => $salaryKpis['average_salary'],
                    'salary_change' => $salaryKpis['payroll_cost_change'],
                    'active_projects' => $projectKpis['active_projects'],
                    'task_completion_rate' => $attendanceKpis['attendance_rate'], // Using attendance as proxy or keep previous
    ...
    private function getAttendanceKpis(array $filteredEmployeeIds, Carbon $start, Carbon $end, Carbon $prevStart, Carbon $prevEnd): array
    {
        ...
        $currentRate = $attendanceStats->total > 0
        ...
        $prevRate = $prevAttendanceStats->total > 0
            ? round(($prevAttendanceStats->attended / $prevAttendanceStats->total) * 100, 1)
            : 0;

        return [
            'attendance_rate' => $currentRate,
            'attendance_change' => round($currentRate - $prevRate, 1),
        ];
    }

    private function getSalaryKpis(array $filteredEmployeeIds, Carbon $start, Carbon $end, Carbon $prevStart, Carbon $prevEnd): array
    {
        ...
        $employeesPaid = (int) $salaryStats->employees_paid;
        $avgSalary = $employeesPaid > 0 ? round($currentTotalSalary / $employeesPaid, 2) : 0;
        ...
        $payrollChange = $prevTotalSalary > 0
            ? round((($currentTotalSalary - $prevTotalSalary) / $prevTotalSalary) * 100, 1)
            : 0;

        return [
            'total_payroll_cost' => $currentTotalSalary,
            'payroll_cost_change' => $payrollChange,
            'average_salary' => $avgSalary,
            'employees_paid' => $employeesPaid,
        ];
    }

    private function getProjectKpis(array $filteredEmployeeIds): array
    {
        $activeProjects = DB::table('projects')
            ->whereIn('status', ['in_progress', 'on_hold'])
            ->count();

        $tasksDone = DB::table('project_tasks')
        ...
        return [
            ...
            'tasks_completed_this_month' => $tasksDone,
        ];
    }
    ...
    private function getLeaveKpis(array $filteredEmployeeIds, int $totalEmployees, Carbon $start, Carbon $end): array
    {
        ...
        $leaveUtilization = $totalEmployees > 0
            ? round(($totalLeaveDays / ($totalEmployees * 22)) * 100, 1) // Assuming 22 working days
            : 0;
    ...
    private function getTeamPerformanceData(Carbon $start, Carbon $end, ?string $department, ?int $teamId): array
    {
        return DB::table('teams')
            ...
            ->map(fn ($t) => [
                ...
                'attendance_rate' => (float) $t->attendance_rate,
                'task_completion_rate' => (float) $t->task_completion_rate,
            ])
            ->toArray();
    }
}
