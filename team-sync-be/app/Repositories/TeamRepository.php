...
class TeamRepository implements TeamRepositoryInterface
{
    ...
    public function create(array $data): Team
    {
        return DB::transaction(function () use ($data) {
            ...
            if ($newLeader instanceof User) {
                DB::afterCommit(function () use ($team, $newLeader, $actorUserId, $actorName) {
                    $this->emailService->sendTeamLeadChangedNotification(
                        $team,
                        null,
                        $newLeader,
                        $actorUserId,
    ...
    public function update(string $id, array $data): Team
    {
        return DB::transaction(function () use ($id, $data) {
            ...
            $oldStatus = (string) ($team->status->value ?? $team->status ?? '');
            $oldLeader = $oldLeaderId ? User::query()->find($oldLeaderId) : null;
            $newLeader = null;
            ...
            if ($team->team_lead_id !== $oldLeaderId) {
                DB::afterCommit(function () use ($team, $oldLeader, $newLeader, $actorUserId, $actorName) {
                    $this->emailService->sendTeamLeadChangedNotification(
                        $team,
                        $oldLeader,
                        $newLeader,
            ...
            $newStatus = (string) ($team->status->value ?? $team->status ?? '');

            if ($newStatus !== '' && $newStatus !== $oldStatus) {
                DB::afterCommit(function () use ($team, $oldStatus, $newStatus, $actorUserId, $actorName) {
                    $this->emailService->sendTeamStatusChangedNotification(
                        $team,
                        $oldStatus,
                        $newStatus,
                        $actorUserId,
    ...
    public function addMember(string $teamId, int $employeeId): TeamMember
    {
        return DB::transaction(function () use ($teamId, $employeeId) {
            ...
            DB::afterCommit(function () use ($team, $member, $actorUserId, $actorName) {
                $this->emailService->sendTeamMemberAddedNotification(
                    $team,
                    $member,
                    $actorUserId,
                    $actorName,
    ...
    public function removeMember(string $teamId, int $employeeId): TeamMember
    {
        return DB::transaction(function () use ($teamId, $employeeId) {
            ...
            DB::afterCommit(function () use ($team, $member, $actorUserId, $actorName) {
                $this->emailService->sendTeamMemberRemovedNotification(
                    $team,
                    $member,
                    $actorUserId,
                    $actorName,
