...
- Meeting broadcasts/reminders use the dedicated `meetings` queue; workers must listen to `default,meetings`
- Meeting reminders require the Laravel scheduler (`php artisan schedule:work` locally or production cron for `schedule:run` every minute)
...
- **Backend dev**: `composer dev` (DO NOT run `artisan serve` manually — `composer dev` handles server + `default,meetings` queue + scheduler + logs + Vite)
...
- **Docker queue/scheduler**: `docker compose up -d queue scheduler` (alternative to `composer dev` queue + scheduler)
...
- **In E2E / manual testing**: Queue worker MUST be running (`composer dev` or `docker compose up -d queue scheduler`); meeting reminders also require the scheduler
