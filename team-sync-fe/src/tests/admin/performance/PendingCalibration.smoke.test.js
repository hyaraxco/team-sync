...
const { routeState, routerPushMock, performanceReviewStoreMock, performanceReviewRefs, routerLinkStub } = vi.hoisted(
    () => ({
        routeState: {
...
describe("PendingCalibration smoke", () => {
    ...
    it("calls fetchPendingCalibration on mount", () => {
        factory();
        expect(performanceReviewStoreMock.fetchPendingCalibration).toHaveBeenCalled();
    });
        ...
        performanceReviewRefs.pendingCalibrationReviews.value = [
            {
                ...
                staff_member: { full_name: "Jane Smith", email: "jane@example.com" },
