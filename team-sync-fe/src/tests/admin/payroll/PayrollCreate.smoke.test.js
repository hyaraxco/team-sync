...
describe("PayrollCreate smoke", () => {
    ...
    it("shows readiness warning returned by backend before submit", async () => {
        payrollStore.fetchGenerateReadiness = vi.fn(async () => ({
            ...
            message: "Payroll for April 2026 can only be generated after the attendance cut-off date on 25 April 2026.",
        }));
    ...
    it("filters readiness employees and opens attendance workspace", async () => {
        ...
        expect(wrapper.find('[data-testid="payroll-create-readiness-dashboard"]').exists()).toBe(true);
        ...
        await wrapper.find('[data-testid="payroll-readiness-open-attendance-99"]').trigger("click");
