...
describe("PayrollSettings smoke", () => {
  ...
  it("shows version comparison details for selected settings history entry", async () => {
    ...
    expect(
      wrapper.find('[data-testid="payroll-settings-history-compare-panel"]').exists(),
    ).toBe(true);
    expect(
      wrapper.findAll('[data-testid="payroll-settings-history-compare-row"]').length,
    ).toBeGreaterThan(0);

    await wrapper
      .get('[data-testid="payroll-settings-history-compare-select-7"]')
      .trigger("click");

    await flushAsync();

    expect(
      wrapper.find('[data-testid="payroll-settings-history-compare-empty"]').exists(),
    ).toBe(true);
  });
});
