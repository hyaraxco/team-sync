...
describe("QuickActions smoke", () => {
    ...
    it("shows admin actions in order and promotes the first actionable item", () => {
        ...
        expect(actions).toEqual(["Add Staff Member", "Create New Team", "Process Payroll", "Schedule Meeting"]);

        expect(wrapper.find('[data-action-id="add-employee"]').attributes("class")).toContain("blue-gradient");
        expect(wrapper.find('[data-action-id="create-team"]').attributes("class")).not.toContain("blue-gradient");
    });
    ...
    it("shows payroll quick action for HR-style draft permissions", () => {
        ...
        expect(actions).toEqual(["Process Payroll", "Schedule Meeting"]);
        expect(wrapper.find('[data-action-id="process-payroll"]').attributes("data-route-name")).toBe(
    ...
    it("shows employee actions without admin actions", () => {
        setPermissions(["attendance-check-in", "attendance-check-out", "leave-request-create"]);
        ...
        expect(wrapper.find('[data-action-id="request-leave"]').attributes("data-route-name")).toBe(
            "staffMember.attendance.my-attendances",
        );
        expect(wrapper.find('[data-action-id="request-leave"]').attributes("data-route-action")).toBe("request-leave");
    });
