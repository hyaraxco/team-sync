...
describe("Header smoke", () => {
  ...
  it("shows actionable dropdown items and removes placeholders", async () => {
    ...
    expect(wrapper.get('[data-testid="header-profile-toggle"]').attributes("aria-haspopup")).toBe(
      "menu"
    );
    expect(wrapper.get('[data-testid="header-profile-toggle"]').attributes("aria-expanded")).toBe(
      "true"
    );
    expect(wrapper.get('[data-testid="header-notification-toggle"]').attributes("aria-haspopup")).toBe(
      "dialog"
    );
