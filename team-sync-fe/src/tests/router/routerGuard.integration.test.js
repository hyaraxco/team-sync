...
const setAuth = ({ token = "test-token", permissions = [], withUser = true } = {}) => {
...
describe("router guard integration", () => {
    ...
    it("keeps project detail and edit routes deterministic after hardening", async () => {
        setAuth({
            permissions: ["dashboard-menu", "project-menu", "project-list", "project-create", "project-edit"],
        });
        const router = createTestRouter();
    ...
    it("allows HR to access payroll create and denies finance on the same route", async () => {
        ...
        setAuth({
            permissions: ["dashboard-menu", "payroll-menu", "payroll-list", "payroll-edit", "payroll-process"],
        });
        const financeRouter = createTestRouter();
    ...
    it("enforces payroll detail access while allowing dual-role self-service payroll", async () => {
        setAuth({
            permissions: ["dashboard-menu", "payroll-menu", "payroll-list", "payroll-create", "payslip-view"],
        });
        const hrRouter = createTestRouter();
    ...
    it("enforces attendance split between admin attendance and employee workspace", async () => {
        ...
        expect(employeeRouter.currentRoute.value.name).toBe("staffMember.attendance.my-attendances");
    ...
    it("enforces clock alias route with strict attendance clock permissions", async () => {
        ...
        expect(clockRouter.currentRoute.value.name).toBe("staffMember.attendance.my-attendances");
    ...
    it("allows employee payroll route but blocks employee from admin payroll route", async () => {
        setAuth({
            permissions: ["dashboard-menu", "profile-menu", "attendance-my-attendances", "payslip-view"],
        });
        const router = createTestRouter();
    ...
    it("allows finance hr and manager into personal self-service routes while keeping admin payroll split", async () => {
        ...
        expect(financeRouter.currentRoute.value.name).toBe("staffMember.attendance.my-attendances");
        ...
        expect(hrRouter.currentRoute.value.name).toBe("staffMember.attendance.my-attendances");
