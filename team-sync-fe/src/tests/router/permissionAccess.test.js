...
import { hasRoutePermissionAccess, normalizePermissions } from "@/router/permissionAccess";

describe("permissionAccess", () => {
    it("normalizes string and object permission payloads", () => {
        expect(normalizePermissions(["payroll-list", { name: "payroll-process" }, null])).toEqual([
            "payroll-list",
            "payroll-process",
        ]);
    });
