...
const routeMeta = (name) =>
  attendanceRoutes.find((route) => route.name === name)?.meta ?? {};

describe("attendance route access", () => {
  it("requires attendance-menu for admin attendance list", () => {
    expect(
      hasRoutePermissionAccess(["attendance-menu"], routeMeta("admin.attendances"))
    ).toBe(true);
    expect(
      hasRoutePermissionAccess(["attendance-list"], routeMeta("admin.attendances"))
    ).toBe(false);
  });
