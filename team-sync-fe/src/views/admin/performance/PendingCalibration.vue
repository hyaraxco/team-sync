...
const { pendingCalibrationReviews, pendingCalibrationLoading, pagination } = storeToRefs(reviewStore);

const selectedCycle = ref("all");
const searchQuery = ref("");

const filteredReviews = computed(() => {
    let reviews = pendingCalibrationReviews.value;
...
const cycles = computed(() => {
    const uniqueCycles = [...new Set(pendingCalibrationReviews.value.map((r) => r.cycle))];
...
const totalPending = computed(() => {
    return pendingCalibrationReviews.value.length;
});

const uniqueCyclesCount = computed(() => {
...
onMounted(async () => {
    await reviewStore.fetchPendingCalibration();
});
</script>
            ...
            <MainCard class="bg-gradient-to-br from-purple-50 to-purple-100 border-purple-200">
                <div class="flex items-center justify-between">
                    ...
                    <div class="w-12 h-12 bg-purple-200 rounded-full flex items-center justify-center">
                        <Calendar class="w-6 h-6 text-purple-700" />
                    </div>
                </div>
            </MainCard>
        </div>
                ...
                <table class="w-full">
                    <thead>
                        <tr class="border-b border-gray-200">
                            ...
                            <th class="text-left py-3 px-4 text-sm font-semibold text-brand-dark">Reviewer</th>
                            <th class="text-left py-3 px-4 text-sm font-semibold text-brand-dark">Review Cycle</th>
                            <th class="text-left py-3 px-4 text-sm font-semibold text-brand-dark">Current Rating</th>
