...
const canSubmitSelfAssessment = computed(() => {
    return reviewStatus.value === "pending_self" && currentEmployeeId.value === review.value?.staff_member_id;
});
...
onMounted(async () => {
    ...
    await Promise.all([reviewStore.fetchReviewById(reviewId.value), reviewStore.fetchActiveSections()]);
    initFormData();

    if (currentReview.value?.status === "pending_calibration" && canCalibrate.value) {
...
<template>
    <div class="space-y-6">
        ...
        <!-- Error Alert -->
        <Alert v-if="error" type="danger" title="Error" :message="error" :show="!!error" />

        <!-- Success Alert -->
        <Alert v-if="success" type="success" title="Success" :message="success" :show="!!success" />
        ...
        <template v-else-if="review">
                ...
                >
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                        ...
                        >
                            <p
                                class="text-xs mb-1"
                                :class="review.promotion_eligible ? 'text-purple-600' : 'text-gray-500'"
                            >
                                Promotion
                            </p>
                            <p
                                class="text-lg font-bold"
                                :class="review.promotion_eligible ? 'text-purple-700' : 'text-gray-400'"
                            >
                        ...
                        <div
                            class="p-3 rounded-lg border text-center"
                            :class="review.pip_required ? 'bg-red-50 border-red-100' : 'bg-gray-50 border-gray-100'"
                        >
                            <p class="text-xs mb-1" :class="review.pip_required ? 'text-red-600' : 'text-gray-500'">
                                PIP
                            </p>
                            <p
                                class="text-lg font-bold"
                                :class="review.pip_required ? 'text-red-700' : 'text-gray-400'"
                            >
                ...
                >
                    ...
                    <div v-else-if="readinessResult?.summary" class="grid grid-cols-2 md:grid-cols-5 gap-3">
                        ...
                        <div
                            ...
                            :class="readinessResult.summary.goals_count > 0 ? 'bg-emerald-50' : 'bg-amber-50'"
                        >
                            <p
                                class="text-xs uppercase tracking-wide font-medium"
                                :class="readinessResult.summary.goals_count > 0 ? 'text-emerald-600' : 'text-amber-600'"
                            >
                                Goals Total
                            </p>
                            <p
                                class="text-2xl font-bold mt-1"
                                :class="readinessResult.summary.goals_count > 0 ? 'text-emerald-800' : 'text-amber-800'"
                            >
                        ...
                        <div
                            class="p-3 rounded-lg"
                            :class="readinessResult.summary.goals_completed > 0 ? 'bg-emerald-50' : 'bg-gray-50'"
                        >
                            <p
                                class="text-xs uppercase tracking-wide font-medium"
                                :class="
                                    readinessResult.summary.goals_completed > 0 ? 'text-emerald-600' : 'text-gray-500'
                                "
                            >
                                Goals Completed
                            </p>
                            <p
                                class="text-2xl font-bold mt-1"
                                :class="
                                    readinessResult.summary.goals_completed > 0 ? 'text-emerald-800' : 'text-gray-400'
                                "
                            >
                        ...
                        <div
                            class="p-3 rounded-lg"
                            :class="readinessResult.summary.goals_on_time > 0 ? 'bg-teal-50' : 'bg-gray-50'"
                        >
                            <p
                                class="text-xs uppercase tracking-wide font-medium"
                                :class="readinessResult.summary.goals_on_time > 0 ? 'text-teal-600' : 'text-gray-500'"
                            >
                                Goals On-Time
                            </p>
                            <p
                                class="text-2xl font-bold mt-1"
                                :class="readinessResult.summary.goals_on_time > 0 ? 'text-teal-800' : 'text-gray-400'"
                            >
                        ...
                        <div
                            class="p-3 rounded-lg"
                            :class="
                                readinessResult.summary.positive_feedback_count > 0 ? 'bg-purple-50' : 'bg-amber-50'
                            "
                        >
                            <p
                                class="text-xs uppercase tracking-wide font-medium"
                                :class="
                                    readinessResult.summary.positive_feedback_count > 0
                                        ? 'text-purple-600'
                                        : 'text-amber-600'
                                "
                            >
                            ...
                            <p
                                v-if="readinessResult.summary.positive_feedback_count === 0"
            ...
            <div v-show="activeTab === 'self-assessment'" class="space-y-6">
                ...
                <template v-else>
                    <!-- Section Cards -->
                    <MainCard v-for="section in displaySections" :key="'self-' + section.id">
                        <div class="space-y-4">
                            <!-- Section Header -->
                                    ...
                                    <p class="text-xs text-brand-light mt-1">
                                        {{
                                            getSelfAssessmentForm(section.id).rating
                                                ? getRatingLabel(getSelfAssessmentForm(section.id).rating)?.label
                                                : "Select a rating (1-5)"
                                        }}
                                    </p>
                                </div>
                                ...
                                <div v-if="getResponseForSection(section.id)?.self_rating">
                                    ...
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span
                                            ...
                                            :class="
                                                getRatingLabel(getResponseForSection(section.id)?.self_rating)?.color
                                            "
                                        >
                        ...
                        <div class="flex flex-wrap gap-3">
                            ...
                            >
                                ...
                                <span
                                    ...
                                    :class="getRatingLabel(getResponseForSection(section.id)?.self_rating)?.color"
                                >
                            ...
                            >
                                ...
                                <div class="flex items-center gap-2">
                                    <span class="text-sm font-bold text-yellow-800">
                                        {{ getResponseForSection(section.id)?.self_rating }}/5
                                    </span>
                                    ...
                                    <p class="text-xs text-brand-light mt-1">
                                        {{
                                            getManagerAssessmentForm(section.id).rating
                                                ? getRatingLabel(getManagerAssessmentForm(section.id).rating)?.label
                                                : "Select a rating (1-5)"
                                        }}
                                    </p>
                                </div>
                                ...
                                <div v-if="getResponseForSection(section.id)?.manager_rating">
                                    ...
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span
                                            class="text-sm font-medium"
                                            :class="
                                                getRatingLabel(getResponseForSection(section.id)?.manager_rating)?.color
                                            "
                                        >
                                            {{
                        ...
                        <div class="space-y-4">
                                ...
                                <div class="p-3 bg-blue-50 border border-blue-100 rounded-lg">
                                    ...
                                    >
                                        <span class="text-lg font-bold text-blue-800">
                                            {{ getResponseForSection(section.id)?.manager_rating }}/5
                                        </span>
                            ...
                            <div v-if="canCalibrate">
                                    ...
                                    <button
                                        ...
                                        @click="
                                            calibrationForm[section.id] = {
                                                rating: calibrationForm[section.id]?.rating === n ? null : n,
                                            }
                                        "
                                ...
                                <div class="mt-3 p-3 bg-purple-50 border border-purple-100 rounded-lg">
                                    <p class="text-xs font-medium text-purple-700 mb-1">Effective Calibration Score</p>
                                    <div class="flex items-center gap-2">
                                        ...
                                        >
                                            {{
                                                getRatingLabel(getCalibrationEffectiveRating(section.id))?.label || "-"
                                            }}
                                        </span>
                                        <span class="text-xs text-purple-600">
                                            Contribution: {{ getCalibrationWeightedContribution(section).toFixed(2) }}
                                        </span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div v-else-if="getResponseForSection(section.id)?.final_rating">
                                ...
                                <div class="flex items-center gap-2">
                                    ...
                                    <span
                                        class="px-2 py-0.5 text-xs font-medium rounded-full border"
                                        :class="getRatingLabel(getResponseForSection(section.id)?.final_rating)?.bg"
                                    >
                                        {{
                                            getRatingLabel(getResponseForSection(section.id)?.final_rating)?.label ||
                                            "-"
                                        }}
                                    </span>
                                </div>
                        ...
                        <div class="space-y-6">
                            ...
                            <div v-if="calibrationContext && !calibrationContextLoading">
                                ...
                                <div class="p-4 bg-gray-50 border border-gray-200 rounded-xl space-y-3">
                                    ...
                                    >
                                        <p class="text-xs font-semibold text-brand-dark mb-2 uppercase tracking-wider">
                                            Manager Breakdown
                                        </p>
                                        <div class="space-y-2">
                                            <div
                                            ...
                                            >
                                                ...
                                                <div class="flex items-center gap-3 text-sm">
                                                    ...
                                                    <span class="font-semibold text-brand-dark">
                                                        Avg: {{ manager.avg_rating || "-" }}
                                                    </span>
                                                    <span class="text-xs text-brand-light">
                                                        {{ manager.min_rating }}-{{ manager.max_rating }}
                                                    </span>
                                                </div>
                                            </div>
                                        </div>
