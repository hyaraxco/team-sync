    ...
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="text-brand-light mt-1">
          Track your performance evaluations and growth
        </p>
      </div>
    </div>
          ...
          <div class="flex-1">
            <div class="flex items-center gap-3 mb-2">
              ...
              <span
                ...
                :class="{
                  'bg-yellow-100 text-yellow-700 border-yellow-200': statusConfig[review.status]?.color === 'warning',
                  'bg-blue-100 text-blue-700 border-blue-200': statusConfig[review.status]?.color === 'info',
                  'bg-emerald-100 text-emerald-700 border-emerald-200': statusConfig[review.status]?.color === 'success',
                  'bg-red-100 text-red-700 border-red-200': statusConfig[review.status]?.color === 'danger',
                }"
              >
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4">
              <div>
                ...
                <p class="text-sm font-medium text-brand-dark mt-1">
                  {{
                    new Date(
                      review.cycle?.review_period_start,
                    ).toLocaleDateString()
                  }}
                  -
                  {{
                    new Date(
                      review.cycle?.review_period_end,
                    ).toLocaleDateString()
                  }}
                </p>
              </div>
              ...
              <div>
                ...
                <div class="flex items-center gap-2 mt-1">
                  <span
                    ...
                    :class="getRatingColor(review.final_rating)"
                  >
                    {{
            ...
            >
              ...
              <p class="text-sm font-medium text-yellow-800">
                ...
                {{
                  new Date(
                    review.cycle?.self_assessment_deadline,
                  ).toLocaleDateString()
                }}
              </p>
            </div>
