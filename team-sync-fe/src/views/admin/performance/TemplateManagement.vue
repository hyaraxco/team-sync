        ...
        <table class="w-full">
          <thead>
            <tr class="border-b border-gray-200">
              ...
              <th class="text-left py-3 px-4 text-xs font-semibold text-brand-light uppercase">Sections</th>
              <th class="text-center py-3 px-4 text-xs font-semibold text-brand-light uppercase">Default</th>
              <th class="text-center py-3 px-4 text-xs font-semibold text-brand-light uppercase">Status</th>
              <th class="text-right py-3 px-4 text-xs font-semibold text-brand-light uppercase">Actions</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="tpl in templates" :key="tpl.id" class="border-b border-gray-100 hover:bg-gray-50">
              ...
              <td class="py-3 px-4 text-center">
                <span :class="tpl.is_active ? 'text-emerald-600' : 'text-gray-400'" class="text-xs font-medium">
                  {{ tpl.is_active ? 'Active' : 'Inactive' }}
                </span>
              </td>
              <td class="py-3 px-4 text-right">
                <div class="flex items-center justify-end gap-2">
                  <button @click="openEdit(tpl)" class="p-1.5 text-brand-light hover:text-brand-primary rounded-lg">
                    <Edit3 class="w-4 h-4" />
                  </button>
                  <button @click="deleteTemplate(tpl)" class="p-1.5 text-brand-light hover:text-red-600 rounded-lg">
    ...
    <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/50 backdrop-blur-sm">
      <div class="bg-white rounded-xl shadow-xl w-full max-w-4xl overflow-hidden flex flex-col max-h-[90vh]">
        <div class="flex items-center justify-between p-4 border-b">
          <h3 class="text-lg font-bold text-brand-dark">{{ editingTemplate ? 'Edit Template' : 'New Template' }}</h3>
        ...
        <div class="flex-1 overflow-y-auto p-6 flex gap-8">
          ...
          <div class="w-1/3 space-y-4">
            ...
            <div>
              ...
              <textarea v-model="form.description" rows="3" class="w-full px-3 py-2 border rounded-lg focus:ring-brand-primary" placeholder="Template usage details..."></textarea>
            </div>
          ...
          <div class="flex-1 space-y-4">
            <div class="flex items-center justify-between">
              ...
              <div :class="totalWeight === 100 ? 'text-emerald-600' : 'text-red-600'" class="text-sm font-bold">
                Total: {{ totalWeight }}%
              </div>
            </div>

            <div v-if="!form.sections.length" class="text-center py-12 border-2 border-dashed rounded-xl bg-gray-50">
              <p class="text-sm text-brand-light">No sections added yet. Select from available sections on the left.</p>
            </div>

            <div v-else class="space-y-3">
              <div v-for="mappedSection in form.sections" :key="mappedSection.id" class="p-4 border rounded-xl bg-white shadow-sm flex items-center gap-4">
                <div class="flex-1">
                  <div class="font-medium text-brand-dark">{{ sections.find(s => s.id === mappedSection.id)?.name }}</div>
                  <div class="text-xs text-brand-light uppercase">{{ sections.find(s => s.id === mappedSection.id)?.topsis_category }}</div>
                </div>
                <div class="w-24">
                  <div class="relative">
                    <input v-model.number="mappedSection.weight" type="number" min="0" max="100" class="w-full pl-3 pr-8 py-2 border rounded-lg text-right" />
                    <span class="absolute right-3 top-2 text-gray-400">%</span>
                  </div>
                </div>
                <button @click="removeSection(mappedSection.id)" class="p-2 text-gray-400 hover:text-red-600 transition-colors">
                  <Trash2 class="w-4 h-4" />
                </button>
              </div>
            </div>

            <div v-if="totalWeight !== 100 && form.sections.length" class="p-3 bg-amber-50 border border-amber-200 rounded-lg flex items-center gap-3 text-amber-700 text-sm">
        ...
        <div class="p-4 border-t bg-gray-50 flex justify-end gap-3">
          <button @click="closeModal" class="px-4 py-2 text-sm font-medium hover:bg-gray-200 rounded-lg transition-colors">Cancel</button>
          <button @click="saveTemplate" :disabled="saving || totalWeight !== 100" class="px-6 py-2 text-sm font-bold text-white bg-brand-primary hover:bg-brand-primary-dark rounded-lg disabled:opacity-50 transition-all">
