...
const { currentCycle, cyclesLoading, topsisResult, topsisLoading } =
...
const totalWeight = computed(() =>
  Object.values(weights.value).reduce((s, v) => s + Number(v), 0)
);
...
const calculateRanking = async () => {
  ...
  try {
    ...
    topsisError.value =
      e?.response?.data?.message ?? "Failed to calculate ranking. Ensure the cycle is completed.";
  }
};
      ...
      <div class="flex-1">
        ...
        <template v-else-if="currentCycle">
          ...
          <p class="text-brand-light mt-1 text-sm">
            Review period:
            {{ new Date(currentCycle.review_period_start).toLocaleDateString("id-ID", { day: "numeric", month: "long", year: "numeric" }) }}
            –
            {{ new Date(currentCycle.review_period_end).toLocaleDateString("id-ID", { day: "numeric", month: "long", year: "numeric" }) }}
          </p>
        </template>
      </div>
    </div>
      ...
      <MainCard>
        ...
        <p class="text-sm text-brand-light">
          {{ currentCycle.self_assessment_deadline
              ? new Date(currentCycle.self_assessment_deadline).toLocaleDateString("id-ID", { day: "numeric", month: "long", year: "numeric" })
              : "Not set" }}
        </p>
      </MainCard>
      ...
      <MainCard>
        ...
        <p class="text-sm text-brand-light">
          {{ currentCycle.manager_assessment_deadline
              ? new Date(currentCycle.manager_assessment_deadline).toLocaleDateString("id-ID", { day: "numeric", month: "long", year: "numeric" })
              : "Not set" }}
        </p>
      </MainCard>
    </div>
    ...
    <template v-if="currentCycle">
      ...
      >
        ...
        <p class="text-sm text-yellow-800">
          TOPSIS Ranking is only available after the cycle is <strong>Completed</strong> and all reviews are calibrated.
          Current status: <strong>{{ cycleStatusConfig[currentCycle.status]?.label }}</strong>
        </p>
      </div>
      ...
      <div v-else class="space-y-4">
        ...
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-3">
            <div class="w-10 h-10 bg-gradient-to-br from-brand-primary to-blue-600 rounded-xl flex items-center justify-center">
              <Award class="w-5 h-5 text-white" />
            </div>
            <div>
              <h2 class="text-xl font-bold text-brand-dark">
                TOPSIS Performance Ranking
              </h2>
              <p class="text-sm text-brand-light">
            ...
            <table class="w-full">
              <thead>
                <tr class="border-b border-gray-200">
                  <th class="text-left py-3 px-3 text-xs font-semibold text-brand-light uppercase tracking-wide w-12">Rank</th>
                  <th class="text-left py-3 px-3 text-xs font-semibold text-brand-light uppercase tracking-wide">Employee</th>
                  <th class="text-center py-3 px-2 text-xs font-semibold text-brand-light uppercase tracking-wide">C1</th>
                  <th class="text-center py-3 px-2 text-xs font-semibold text-brand-light uppercase tracking-wide">C2</th>
                  ...
                  >
                    ...
                    <td class="py-4 px-3">
                      <span
                        ...
                        :class="rankMedalColor(item.rank)"
                      >
                  ...
                  <tr v-if="expandedRows.has(item.staff_member_id)" class="bg-blue-50/40">
                    <td colspan="10" class="px-6 py-4">
                      <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                        <!-- Normalisasi steps -->
                        <div>
                          <p class="font-semibold text-brand-dark mb-2">📐 TOPSIS Calculation Detail</p>
                          <table class="w-full text-xs">
                            ...
                            <tbody>
                              <tr v-for="(label, key) in weightLabels" :key="key" class="border-t border-blue-100">
                                <td class="py-1 text-brand-light">{{ label }}</td>
                                <td class="py-1 text-right font-mono text-brand-dark">
                                  {{ typeof item.raw_scores[key] === 'number' ? item.raw_scores[key].toFixed(4) : item.raw_scores[key] }}
                                </td>
                        ...
                        <!-- Jarak & Skor akhir -->
                        <div>
                          <p class="font-semibold text-brand-dark mb-2">📏 Distance & Closeness Coefficient</p>
                          <div class="space-y-2 text-xs">
                            <div class="flex justify-between p-2 bg-white rounded border border-blue-100">
                              <span class="text-brand-light">D⁺ (distance to positive ideal)</span>
                              <span class="font-mono text-red-600">{{ item.distance_positive.toFixed(6) }}</span>
                            </div>
                            <div class="flex justify-between p-2 bg-white rounded border border-blue-100">
                              <span class="text-brand-light">D⁻ (distance to negative ideal)</span>
                              <span class="font-mono text-green-600">{{ item.distance_negative.toFixed(6) }}</span>
                            </div>
                            <div class="flex justify-between p-2 bg-brand-primary/10 rounded border border-brand-primary/20">
                              <span class="font-semibold text-brand-dark">Ci = D⁻ / (D⁺ + D⁻)</span>
              ...
              <div class="p-3 bg-green-50 rounded-lg border border-green-100">
                ...
                <div class="space-y-1">
                  <div v-for="(label, key) in weightLabels" :key="key"
                    ...
                    <span>{{ label }}</span>
                    <span class="font-mono">{{ topsisResult.ideal_positive[key]?.toFixed(6) }}</span>
                  </div>
                </div>
              </div>
              <div class="p-3 bg-red-50 rounded-lg border border-red-100">
                ...
                <div class="space-y-1">
                  <div v-for="(label, key) in weightLabels" :key="key"
                    ...
                    <span>{{ label }}</span>
                    <span class="font-mono">{{ topsisResult.ideal_negative[key]?.toFixed(6) }}</span>
                  </div>
                </div>
              </div>
            </div>
        ...
        <MainCard v-else-if="!topsisLoading && !topsisError">
          <div class="text-center py-8">
            ...
            <p class="text-brand-light text-sm mt-1">
              Click "Recalculate" to run the TOPSIS algorithm.<br />
              Make sure there are reviews with <strong>completed</strong> status in this cycle.
            </p>
          </div>
        </MainCard>
      </div>
