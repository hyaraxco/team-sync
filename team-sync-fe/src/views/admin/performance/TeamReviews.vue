...
const averageRating = computed(() => {
    const completedWithRatings = teamReviews.value.filter((r) => r.status === "completed" && r.final_rating);
    if (completedWithRatings.length === 0) return 0;
    const sum = completedWithRatings.reduce((acc, r) => acc + parseFloat(r.final_rating), 0);
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <MainCard class="bg-success-50 border-success-100">
                <div class="flex items-center justify-between">
                    ...
                    <div class="w-12 h-12 bg-green-200 rounded-full flex items-center justify-between">
            ...
            <MainCard class="bg-primary-50 border-primary-100">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-bold text-blue-900 mt-2">{{ averageRating }} / 5.00</p>
                    </div>
            ...
            <div class="space-y-4">
                <div class="flex flex-wrap gap-4">
                    <div class="flex-1 min-w-[250px]">
                        <label class="block text-sm font-medium text-brand-dark mb-2">Search Employee</label>
                        <input
                            v-model="searchQuery"
                            type="text"
                ...
                <table class="w-full">
                    <thead>
                        <tr class="border-b border-gray-200">
                            <th class="text-left py-3 px-4 text-sm font-semibold text-brand-dark">Employee</th>
                            <th class="text-left py-3 px-4 text-sm font-semibold text-brand-dark">Review Cycle</th>
                            <th class="text-left py-3 px-4 text-sm font-semibold text-brand-dark">Status</th>
                            <th class="text-left py-3 px-4 text-sm font-semibold text-brand-dark">Self-Assessment</th>
                            <th class="text-left py-3 px-4 text-sm font-semibold text-brand-dark">Overall Score</th>
                            <th class="text-right py-3 px-4 text-sm font-semibold text-brand-dark">Actions</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr
                        ...
                        >
                            ...
                            <td class="py-4 px-4">
                                <div class="flex items-center gap-2">
                                    ...
                                    <span class="text-sm text-brand-dark">{{ review.cycle?.name || "-" }}</span>
                                </div>
                            </td>
                            <td class="py-4 px-4">
                                <StatusBadge :value="review.status" :label="statusConfig[review.status]?.label" />
                            ...
                            <td class="py-4 px-4">
                                <div v-if="review.final_rating" class="flex items-center gap-2">
                                    <span class="text-lg font-bold text-brand-dark">{{ review.final_rating }}</span>
                            ...
                            <td class="py-4 px-4 text-right">
                                ...
                                >
                                    {{ review.status === "pending_manager" ? "Review Now" : "View Details" }}
                                </button>
                            </td>
                        </tr>
                    </tbody>
