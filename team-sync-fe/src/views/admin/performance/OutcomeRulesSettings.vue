...
const sortedRules = computed(() =>
  [...(outcomeRules.value || [])].sort((a, b) => a.min_rating - b.min_rating)
);
    ...
    <MainCard>
      ...
      <div v-else-if="!sortedRules.length" class="text-center py-12">
        ...
        <p class="text-sm text-brand-light mt-1 max-w-sm mx-auto">
          Add rules to automatically assign outcomes (bonus, salary increase, promotion) based on review ratings.
        </p>
        <button
          @click="openCreate"
        ...
        <table class="w-full">
          <thead>
            <tr class="border-b border-gray-200">
              <th class="text-left py-3 px-4 text-xs font-semibold text-brand-light uppercase tracking-wider">Label</th>
              <th class="text-left py-3 px-4 text-xs font-semibold text-brand-light uppercase tracking-wider">Rating Range</th>
              <th class="text-left py-3 px-4 text-xs font-semibold text-brand-light uppercase tracking-wider">Bonus</th>
              <th class="text-left py-3 px-4 text-xs font-semibold text-brand-light uppercase tracking-wider">Salary Increase</th>
              <th class="text-center py-3 px-4 text-xs font-semibold text-brand-light uppercase tracking-wider">Promotion</th>
              <th class="text-center py-3 px-4 text-xs font-semibold text-brand-light uppercase tracking-wider">PIP</th>
              <th class="text-center py-3 px-4 text-xs font-semibold text-brand-light uppercase tracking-wider">Active</th>
              <th class="text-right py-3 px-4 text-xs font-semibold text-brand-light uppercase tracking-wider">Actions</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="rule in sortedRules" :key="rule.id" class="border-b border-gray-100 hover:bg-gray-50">
              <td class="py-3 px-4">
              ...
              <td class="py-3 px-4">
                <span class="text-sm text-brand-dark font-mono">{{ rule.min_rating.toFixed(2) }} — {{ rule.max_rating.toFixed(2) }}</span>
              </td>
              <td class="py-3 px-4">
              ...
              <td class="py-3 px-4 text-center">
                <span v-if="rule.promotion_eligible" class="inline-block px-2 py-0.5 rounded text-xs bg-purple-100 text-purple-700 border border-purple-200">Yes</span>
                <span v-else class="text-xs text-gray-400">—</span>
              </td>
              <td class="py-3 px-4 text-center">
                <span v-if="rule.pip_required" class="inline-block px-2 py-0.5 rounded text-xs bg-red-100 text-red-700 border border-red-200">
                  <AlertTriangle class="w-3 h-3 inline -mt-0.5" /> Required
                </span>
                <span v-else class="text-xs text-gray-400">—</span>
              </td>
              <td class="py-3 px-4 text-center">
              ...
              <td class="py-3 px-4 text-right">
                <div class="flex items-center justify-end gap-2">
                  <button @click="openEdit(rule)" class="p-1.5 text-brand-light hover:text-brand-primary hover:bg-gray-100 rounded-lg transition-colors">
                    <Edit3 class="w-4 h-4" />
                  </button>
                  <button @click="deleteRule(rule)" class="p-1.5 text-brand-light hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors">
        ...
        <form @submit.prevent="saveRule" class="p-4 space-y-4 max-h-[70vh] overflow-y-auto">
          ...
          <div class="flex items-center gap-6">
            <label class="flex items-center gap-2 cursor-pointer">
              <input v-model="form.promotion_eligible" type="checkbox" class="rounded border-gray-300 text-brand-primary focus:ring-brand-primary" />
              <span class="text-sm text-brand-dark">Promotion Eligible</span>
            </label>
            <label class="flex items-center gap-2 cursor-pointer">
              <input v-model="form.pip_required" type="checkbox" class="rounded border-gray-300 text-red-600 focus:ring-red-500" />
              <span class="text-sm text-brand-dark">PIP Required</span>
            </label>
            <label class="flex items-center gap-2 cursor-pointer">
              <input v-model="form.is_active" type="checkbox" class="rounded border-gray-300 text-brand-primary focus:ring-brand-primary" />
          ...
          <div class="flex items-center justify-end gap-3 pt-4 border-t border-gray-100">
            <button type="button" @click="closeModal" class="px-4 py-2 text-sm font-medium text-brand-dark hover:bg-gray-200 rounded-lg transition-colors">
              Cancel
            </button>
            <button
              type="submit"
