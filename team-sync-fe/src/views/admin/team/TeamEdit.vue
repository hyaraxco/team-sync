...
const handleSelectLead = (employee) => {
    const { selectedLead: sl, team_lead_id } = teamStore.selectLeadPayload(employee);
...
const addNewResponsibility = () => {
    form.value.responsibilities = teamStore.addResponsibility(form.value.responsibilities);
};

const removeResponsibility = (idx) => {
    form.value.responsibilities = teamStore.removeResponsibility(form.value.responsibilities, idx);
};
