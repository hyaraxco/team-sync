...
const getErrorMap = () => {
  if (!employeeStore.error || typeof employeeStore.error !== "object") {
    return {} as Record<string, string[]>;
  }

  return { ...(employeeStore.error as Record<string, string[]>) };
};

const setFieldError = (field: string, message: string) => {
  const nextErrors = getErrorMap();
  nextErrors[field] = [message];
  employeeStore.error = nextErrors;
};

const clearFieldError = (field: string) => {
  if (!employeeStore.error || typeof employeeStore.error !== "object") return;

  const nextErrors = getErrorMap();
  delete nextErrors[field];
  employeeStore.error = Object.keys(nextErrors).length ? nextErrors : null;
};
...
const checkAvailabilityRealtime = async (
  ...
  try {
    await employeeStore.checkAvailability(payload);
...
const validateStep1 = async () => {
  ...
  if (Object.keys(validationErrors).length > 0) {
    employeeStore.error = validationErrors;
    return false;
  }

  try {
    await employeeStore.checkAvailability({
