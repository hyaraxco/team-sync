    ...
    scheduleItems.value.forEach((schedule) => {
        ...
        overrides.forEach((override) => {
            ...
            rows.push({
                ...
                currentLocation:
                    override?.current_location ||
                    (weekdayKey ? baseSchedule[weekdayKey] : null) ||
                    '-',
            });
        });
    });
...
<template>
                        ...
                        <table class="w-full min-w-[900px]">
                            <thead>
                                <tr class="border-y border-[#EAECF0]">
                                    <th class="py-3 px-3 text-left text-xs font-semibold text-[#667085] uppercase tracking-wide">Employee</th>
                                    <th class="py-3 px-3 text-left text-xs font-semibold text-[#667085] uppercase tracking-wide">Base Schedule (Mon - Fri)</th>
                                    <th class="py-3 px-3 text-left text-xs font-semibold text-[#667085] uppercase tracking-wide">Status</th>
                                </tr>
                            </thead>
                                ...
                                >
                                    ...
                                    <td class="py-4 px-3">
                                        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
                                            ...
                                            >
                                                <span class="text-xs font-semibold capitalize">{{ day.slice(0, 3) }}</span>
                                                <span class="inline-flex items-center gap-1 text-xs font-semibold">
                                                    <component :is="getLocationMeta(getBaseSchedule(schedule)[day]).icon" class="w-3.5 h-3.5" />
                    ...
                    <div v-else class="overflow-x-auto">
                        <table class="w-full min-w-[1100px]">
                            <thead>
                                <tr class="border-y border-[#EAECF0]">
                                    <th class="py-3 px-3 text-left text-xs font-semibold text-[#667085] uppercase tracking-wide">Employee</th>
                                    <th class="py-3 px-3 text-left text-xs font-semibold text-[#667085] uppercase tracking-wide">Requested Date</th>
                                    <th class="py-3 px-3 text-left text-xs font-semibold text-[#667085] uppercase tracking-wide">Current Location</th>
                                    <th class="py-3 px-3 text-left text-xs font-semibold text-[#667085] uppercase tracking-wide">Requested Location</th>
                                    <th class="py-3 px-3 text-left text-xs font-semibold text-[#667085] uppercase tracking-wide">Reason</th>
                                    <th class="py-3 px-3 text-left text-xs font-semibold text-[#667085] uppercase tracking-wide">Status</th>
                                    <th class="py-3 px-3 text-left text-xs font-semibold text-[#667085] uppercase tracking-wide">Actions</th>
                                </tr>
                            </thead>
                            ...
                            <tbody>
                                ...
                                >
                                    <td class="py-4 px-3 text-sm font-semibold text-[#101828]">{{ override.employeeName }}</td>
                                    <td class="py-4 px-3 text-sm text-[#344054]">{{ formatRequestDate(override.date) }}</td>
                                    <td class="py-4 px-3">
                                        <span class="inline-flex items-center gap-1 text-xs font-semibold px-2.5 py-1 rounded-full border" :class="getLocationMeta(override.currentLocation).badgeClass">
                                            <component :is="getLocationMeta(override.currentLocation).icon" class="w-3.5 h-3.5" />
    ...
    >
        <div class="space-y-4">
            ...
            <div>
                <label class="block text-sm font-semibold text-[#1D2939] mb-2">Rejection Notes <span class="text-red-500">*</span></label>
                <textarea
