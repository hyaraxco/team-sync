  ...
  <div class="attendance-settings-container min-h-screen bg-neutral-900 text-neutral-100 p-8">
    <div class="max-w-7xl mx-auto space-y-8 relative">
      ...
      <transition name="fade" mode="out-in">
        <section v-if="activeTab === 'Attendance Policies'" key="policies" class="relative z-10 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          ...
          <div class="policy-card flex flex-col items-center justify-center p-6 rounded-2xl border border-dashed border-white/20 text-neutral-500 bg-white/[0.02] min-h-[300px]">
            <svg class="w-12 h-12 mb-4 opacity-40" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 4v16m8-8H4"></path></svg>
            <span class="font-medium tracking-wide text-center">Custom policy creation is not available from the current API.</span>
            <span class="text-xs text-neutral-600 mt-2 text-center">Edit existing employment-type policies instead.</span>
          </div>
        </section>
            ...
            <table class="w-full text-left border-collapse">
              ...
              <tbody class="divide-y divide-white/5">
                ...
                <tr v-else v-for="holiday in holidayStore.paginatedHolidays" :key="holiday.id" class="hover:bg-white/10 transition-colors">
                  ...
                  <td class="p-4"><span class="px-2 py-1 text-xs rounded bg-neutral-800 text-neutral-300 border border-neutral-700">{{ formatHolidayType(holiday.type) }}</span></td>
    ...
    >
      <form class="space-y-5" @submit.prevent="submitPolicyForm">
        <p class="text-sm text-brand-light mb-2">
          Editing policy for <span class="font-semibold text-brand-dark">{{ formatLabel(selectedPolicy?.employment_type) }}</span>.
        </p>
        ...
        <div class="grid gap-4 md:grid-cols-2">
          ...
          <div>
            ...
            <input v-model.number="policyForm.work_days_per_week" type="number" min="1" max="7" required class="form-input" />
          </div>
          <div>
            <label class="block text-sm font-semibold text-brand-dark mb-2">Late Grace Minutes</label>
            <input v-model.number="policyForm.late_grace_minutes" type="number" min="0" max="120" required class="form-input" />
          </div>
          <div>
            <label class="block text-sm font-semibold text-brand-dark mb-2">Half Day Minimum Hours</label>
            <input v-model.number="policyForm.half_day_min_hours" type="number" min="0" max="12" step="0.25" required class="form-input" />
          </div>
          <div>
            <label class="block text-sm font-semibold text-brand-dark mb-2">Warning Absent %</label>
            <input v-model.number="policyForm.warning_absent_pct" type="number" min="0" max="100" step="0.01" required class="form-input" />
          </div>
        </div>
        ...
        <div>
          ...
          <div class="grid gap-2 sm:grid-cols-2 md:grid-cols-4">
            <label v-for="day in weekdays" :key="day.value" class="flex items-center gap-2 text-sm text-brand-dark border border-[#DCDEDD] rounded-lg px-3 py-2">
              <input v-model="policyForm.default_working_weekdays" type="checkbox" :value="day.value" class="rounded" />
              {{ day.label }}
            </label>
          </div>
        </div>
        ...
        <div class="flex gap-3 pt-2">
          <button type="button" :disabled="isSubmittingPolicy" class="modal-secondary-button" @click="closePolicyModal">
            Cancel
          </button>
    ...
    >
      <form class="space-y-5" @submit.prevent="submitEntitlementForm">
        <p class="text-sm text-brand-light mb-2">
          Editing <span class="font-semibold text-brand-dark">{{ formatLabel(selectedEntitlement?.leave_type) }}</span>
          for <span class="font-semibold text-brand-dark">{{ formatLabel(selectedEntitlement?.employment_type) }}</span>.
        </p>

        <div class="grid gap-3 md:grid-cols-3">
          <label class="flex items-center gap-2 text-sm text-brand-dark border border-[#DCDEDD] rounded-lg px-3 py-2">
        ...
        <div class="grid gap-4 md:grid-cols-2">
          ...
          <div>
            ...
            <input v-model.number="entitlementForm.quota_days" type="number" min="0" step="0.5" class="form-input" />
          </div>
          <div>
            <label class="block text-sm font-semibold text-brand-dark mb-2">Carry Over Max Days</label>
            <input v-model.number="entitlementForm.carry_over_max_days" type="number" min="0" class="form-input" />
          </div>
          <div>
            <label class="block text-sm font-semibold text-brand-dark mb-2">Max Attachment Size (KB)</label>
            <input v-model.number="entitlementForm.max_attachment_size_kb" type="number" min="0" class="form-input" />
          </div>
        </div>

        <div>
          <label class="block text-sm font-semibold text-brand-dark mb-2">Allowed MIME Types</label>
          <input v-model="allowedMimeTypesInput" type="text" placeholder="image/jpeg, image/png, application/pdf" class="form-input" />
          <p class="mt-1 text-xs text-gray-500">Separate MIME types with commas. Leave blank for no restriction.</p>
        </div>

        <div class="flex gap-3 pt-2">
          <button type="button" :disabled="isSubmittingEntitlement" class="modal-secondary-button" @click="closeEntitlementModal">
            Cancel
          </button>
    ...
    >
      <form class="space-y-4" @submit.prevent="submitHolidayForm">
        ...
        <div>
          ...
          <input v-model="holidayForm.name" type="text" required placeholder="e.g., Independence Day" class="form-input" />
        </div>
        <div>
        ...
        <div>
          ...
          <input v-model="holidayAppliesToInput" type="text" placeholder="Optional, comma separated" class="form-input" />
          <p class="mt-1 text-xs text-gray-500">Leave blank to apply company-wide.</p>
        </div>

        <div class="flex gap-3 pt-2">
          <button type="button" :disabled="isSubmittingHoliday" class="modal-secondary-button" @click="closeHolidayModal">
            Cancel
          </button>
...
const commaSeparatedToArray = (value) => String(value || '')
  .split(',')
  .map(item => item.trim())
  .filter(Boolean);
...
const openPolicyModal = (policy) => {
  ...
  policyForm.value = {
    ...
    default_working_weekdays: Array.isArray(policy.default_working_weekdays) ? [...policy.default_working_weekdays] : [],
...
const submitPolicyForm = async () => {
  ...
  try {
    ...
    toast.error('Failed to update policy', policyStore.error || error?.response?.data?.message || 'Failed to save attendance policy.');
  } finally {
...
const openEntitlementModal = (entitlement) => {
  ...
  entitlementForm.value = {
    ...
    carry_over_max_days: entitlement.carry_over_max_days === null ? null : Number(entitlement.carry_over_max_days || 0),
    ...
    max_attachment_size_kb: entitlement.max_attachment_size_kb === null ? null : Number(entitlement.max_attachment_size_kb || 0),
  };
...
const submitEntitlementForm = async () => {
  ...
  try {
    ...
    toast.error('Failed to update entitlement', entitlementStore.error || error?.response?.data?.message || 'Failed to save leave entitlement.');
  } finally {
...
const submitHolidayForm = async () => {
  ...
  try {
    ...
    toast.error('Failed to save holiday', holidayStore.error || error?.response?.data?.message || 'Failed to save holiday.');
  } finally {
...
onMounted(async () => {
  try {
    ...
    toast.error(
      ...
      policyStore.error || entitlementStore.error || holidayStore.error || error?.response?.data?.message || 'Failed to load settings data.',
    );
  }
});
</script>
