...
const notePreview = computed(() => {
    ...
    return previewTemplate
        .replaceAll("{working_days}", String(form.working_days_mode === "fixed" ? form.default_working_days : 22))
...
const selectedHistoryVersionIndex = computed(() =>
    settingsHistory.value.findIndex((version) => version.id === selectedHistoryVersionId.value),
);
...
const handleSubmit = async () => {
    try {
        await payrollStore.updateSettings({
            ...
            rounding_unit: form.rounding_mode === "none" ? 1 : Number(form.rounding_unit || 1),
            note_template: form.note_template?.trim() || fallbackSettings.note_template,
    ...
    <div class="space-y-6">
        ...
        <div class="bg-blue-50 border border-blue-200 rounded-2xl px-5 py-4">
            ...
            <p class="text-blue-800 text-sm mt-1">
                Existing payroll records stay unchanged. Update settings before HR generates the next draft.
            </p>
            <p class="text-blue-700 text-xs mt-2" data-testid="payroll-settings-updated-by">
                {{ lastUpdatedLabel }}
            </p>
        </div>
            ...
            <div class="space-y-6 min-w-0 pb-10">
                ...
                >
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label class="block text-brand-dark text-sm font-semibold mb-2">Default payday</label>
                            <input
                ...
                >
                    ...
                    <div class="space-y-4">
                        <div>
                            <label class="block text-brand-dark text-sm font-semibold mb-2">Working days mode</label>
                            <select
                        ...
                        <div v-if="form.working_days_mode === 'fixed'">
                            <label class="block text-brand-dark text-sm font-semibold mb-2">Default working days</label>
                            <input
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label class="block text-brand-dark text-sm font-semibold mb-2">Rounding mode</label>
                                <select
                            ...
                            <div>
                                <label class="block text-brand-dark text-sm font-semibold mb-2">Rounding unit</label>
                                <input
                ...
                <div v-show="activeTab === 'bank_notes'" class="space-y-6 animate-fade-in">
                    ...
                    >
                        ...
                        <p class="text-brand-light text-xs mt-2">
                            ...
                            `{permission_days}`, `{absent_days}`, `{deduction}`.
                        </p>
                    </section>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div class="border border-brand-border rounded-2xl px-4 py-3 bg-gray-50/50">
                                ...
                                <p class="text-brand-dark text-sm mt-2">
                                    Payday day
                                    <span class="font-bold">{{ form.payday_day }}</span>
                                    , cut-off day
                        ...
                        <template v-for="version in settingsHistory" :key="version.id">
                            ...
                            >
                                ...
                                <button
                                    ...
                                    :class="selectedHistoryVersionId === version.id ? 'text-blue-800' : 'text-blue-600'"
                                >
                                    <span>Compare with previous version</span>
                                    <component
                                        :is="selectedHistoryVersionId === version.id ? ChevronUp : ChevronDown"
                                        class="w-3.5 h-3.5"
                                    />
                                </button>
                            </div>
                            ...
                            >
                                ...
                                >
                                    This is the oldest version in history. No previous version is available for
                                    comparison.
                                </div>
                                ...
                                <div v-else class="mt-3 space-y-2">
                                    ...
                                    >
                                        ...
                                        <p class="text-xs text-brand-light mt-1">
                                            From
                                            <span class="font-semibold text-brand-dark">{{ row.previous }}</span>
                                            to
                                            <span class="font-semibold text-blue-700">{{ row.current }}</span>
                                        </p>
                                    </div>
                                </div>
                            </div>
