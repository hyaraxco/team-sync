...
const sortedPolicies = computed(() => [...policies.value].sort((a, b) => Number(a.approval_order || 0) - Number(b.approval_order || 0)));
...
const loadPolicies = async () => {
    ...
    try {
        ...
        toast.error('Failed to load approval policies', payrollStore.error || error?.response?.data?.message || 'Please try again.');
    } finally {
        loading.value = false;
    }
};
...
const submitForm = async () => {
    ...
    try {
        ...
        toast.error('Failed to save approval policy', payrollStore.error || error?.response?.data?.message || 'Please check the form and try again.');
    } finally {
...
const confirmDelete = async () => {
    ...
    try {
        ...
        toast.error('Failed to delete approval policy', payrollStore.error || error?.response?.data?.message || 'Please try again.');
    } finally {
        isSubmitting.value = false;
    }
};
...
<template>
    <div class="space-y-6">
                    ...
                    <tbody class="divide-y divide-[#F1F5F9] bg-white">
                        <tr v-for="policy in sortedPolicies" :key="policy.id">
                            ...
                            <td class="px-4 py-3 text-brand-dark">
                                {{ formatRupiah(policy.min_amount || 0) }} — {{ policy.max_amount ? formatRupiah(policy.max_amount) : 'No maximum' }}
                            </td>
                            <td class="px-4 py-3 text-brand-dark">{{ policy.required_role }}</td>
                            <td class="px-4 py-3">
                                <span
                                    class="inline-flex rounded-full px-2.5 py-1 text-xs font-semibold"
                                    :class="policy.is_active ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600'"
                                >
                                    {{ policy.is_active ? 'Active' : 'Inactive' }}
                                </span>
                            </td>
                            <td class="px-4 py-3 text-right">
                                <button type="button" class="mr-2 inline-flex items-center gap-1 rounded-lg border border-[#DCDEDD] px-3 py-2 text-xs font-semibold" @click="openEdit(policy)">
                                    <Pencil class="w-3.5 h-3.5" /> Edit
                                </button>
                                <button type="button" class="inline-flex items-center gap-1 rounded-lg border border-red-200 px-3 py-2 text-xs font-semibold text-red-600" @click="openDelete(policy)">
        ...
        <ModalWrapper :show="isFormOpen" :title="isEditing ? 'Edit Approval Policy' : 'Add Approval Policy'" maxWidth="md" @close="closeForm">
            <form class="space-y-4" @submit.prevent="submitForm">
                <div>
                    <label class="block text-sm font-semibold text-brand-dark mb-2">Policy Name</label>
                    <input v-model="form.name" required type="text" class="w-full rounded-[10px] border border-[#DCDEDD] px-4 py-3" />
                </div>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-semibold text-brand-dark mb-2">Minimum Amount</label>
                        <input v-model="form.min_amount" required min="0" type="number" class="w-full rounded-[10px] border border-[#DCDEDD] px-4 py-3" />
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-brand-dark mb-2">Maximum Amount</label>
                        <input v-model="form.max_amount" min="0" type="number" placeholder="No maximum" class="w-full rounded-[10px] border border-[#DCDEDD] px-4 py-3" />
                    </div>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-semibold text-brand-dark mb-2">Required Role</label>
                        <input v-model="form.required_role" required type="text" placeholder="finance-manager" class="w-full rounded-[10px] border border-[#DCDEDD] px-4 py-3" />
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-brand-dark mb-2">Approval Order</label>
                        <input v-model="form.approval_order" required min="1" type="number" class="w-full rounded-[10px] border border-[#DCDEDD] px-4 py-3" />
                    </div>
                </div>
                ...
                <div class="flex justify-end gap-3 pt-2">
                    <button type="button" class="rounded-[10px] border border-[#DCDEDD] px-4 py-3 text-sm font-semibold" @click="closeForm">Cancel</button>
                    <button type="submit" :disabled="isSubmitting" class="rounded-[10px] bg-brand-dark px-4 py-3 text-sm font-semibold text-white disabled:opacity-50">
        ...
        <ModalWrapper :show="isDeleteOpen" title="Delete Approval Policy" maxWidth="md" @close="closeDelete">
            <p class="text-sm text-brand-light mb-6">
                Delete <span class="font-semibold text-brand-dark">{{ selectedPolicy?.name }}</span>? Existing payroll approval records stay unchanged, but future payrolls will not use this policy.
            </p>
            <template #footer>
                <div class="flex gap-3">
                    <button type="button" class="flex-1 rounded-[10px] border border-[#DCDEDD] px-4 py-3 text-sm font-semibold" @click="closeDelete">Cancel</button>
                    <button type="button" :disabled="isSubmitting" class="flex-1 rounded-[10px] bg-red-600 px-4 py-3 text-sm font-semibold text-white disabled:opacity-50" @click="confirmDelete">
