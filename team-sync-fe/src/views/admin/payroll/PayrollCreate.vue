...
import { Calendar, Calculator, Plus, ArrowLeft, AlertCircle, Search } from "lucide-vue-next";
...
const existingSalaryMonths = computed(
    () =>
        new Set(
            payrolls.value
                .map((payroll) => toMonthKey(payroll?.salary_month ?? payroll?.period ?? payroll?.month ?? null))
                .filter(Boolean),
        ),
);

const isMonthAlreadyGenerated = computed(() => existingSalaryMonths.value.has(form.value.salary_month));
...
const hiddenReadinessCount = computed(() =>
    Math.max(0, filteredReadinessEmployees.value.length - visibleReadinessEmployees.value.length),
);

const canToggleReadinessExpansion = computed(() => filteredReadinessEmployees.value.length > readinessPreviewLimit);
...
const preloadExistingPayrollMonths = async () => {
    ...
    if (payrollStore.error) {
        toast.warning("Incomplete payroll data", "Duplicate month validation will be checked upon submission.");
        return;
    }
        ...
        <div class="flex-1">
            <form @submit.prevent="handleSubmit" class="space-y-6">
                ...
                <div class="bg-white border border-brand-border rounded-2xl p-6">
                    ...
                    <div class="mb-4">
                        <label class="block text-sm font-medium text-gray-700 mb-1.5">Salary Month *</label>
                        <div class="relative">
                    ...
                    <div class="mt-6 p-4 bg-blue-50 border border-blue-200 rounded-xl flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-blue-800 text-sm">
                                Payroll will be automatically generated for all active employees based on their
                                attendance records for the selected month. Salaries will be calculated based on
                                attendance, sick days, and absences.
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <div class="bg-white border border-brand-border rounded-2xl p-6 sticky top-6">
                ...
                <div class="space-y-4">
                    ...
                    >
                        ...
                        <p class="text-sm mt-1" :class="readiness.can_generate ? 'text-green-700' : 'text-amber-700'">
                            {{ readiness.message }}
                        </p>
                    </div>
