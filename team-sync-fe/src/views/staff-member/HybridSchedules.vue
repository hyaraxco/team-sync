  ...
  <div class="hybrid-schedules-container min-h-screen bg-neutral-900 text-neutral-100 p-8">
    <div class="max-w-6xl mx-auto space-y-8 relative">
      ...
      <header class="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-end gap-6 border-b border-white/30 pb-8">
        ...
        <button @click="showOverrideModal = true" class="px-6 py-2.5 rounded-full bg-white text-black font-medium text-sm hover:scale-105 transition-transform duration-300 shadow-lg shadow-white/10">
          Request Override
        </button>
      </header>
        ...
        <div v-else class="grid gap-8 lg:grid-cols-2">
          ...
          <div class="space-y-4">
            ...
            <div class="grid gap-3">
              <div v-for="(location, day) in baseSchedule" :key="day"
              ...
              <div v-for="override in schedule.overrides" :key="override.id"
                ...
                <div class="flex justify-between items-start mb-2">
                  <div>
                    ...
                    <p class="text-sm text-neutral-400">Change to: <span class="text-white">{{ override.planned_work_mode || override.requested_location }}</span></p>
                  </div>
    ...
    <div v-if="showOverrideModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <form class="w-full max-w-md rounded-2xl bg-white p-6 text-neutral-900 space-y-4" @submit.prevent="submitOverride">
        <h2 class="text-xl font-semibold">Request Schedule Override</h2>
        <div>
          <label class="block text-sm font-medium mb-1">Date</label>
          <input v-model="overrideForm.date" type="date" required class="w-full rounded-lg border border-neutral-300 px-3 py-2" />
        </div>
        <div>
          <label class="block text-sm font-medium mb-1">Requested Work Mode</label>
          <select v-model="overrideForm.planned_work_mode" required class="w-full rounded-lg border border-neutral-300 px-3 py-2">
        ...
        <div>
          ...
          <textarea v-model="overrideForm.reason" rows="3" required class="w-full rounded-lg border border-neutral-300 px-3 py-2"></textarea>
        </div>
        <div class="flex justify-end gap-3 pt-2">
          <button type="button" class="rounded-lg border border-neutral-300 px-4 py-2" @click="showOverrideModal = false">Cancel</button>
          <button type="submit" :disabled="submittingOverride" class="rounded-lg bg-neutral-900 px-4 py-2 text-white disabled:opacity-50">
...
const baseSchedule = computed(() => schedule.value?.base_schedule || {
