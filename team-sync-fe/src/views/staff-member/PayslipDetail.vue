...
onMounted(async () => {
    try {
        ...
        toast.error(
            ...
            payrollStore.error || error?.response?.data?.message || "Failed to load payslip.",
        );
    } finally {
        loading.value = false;
    }
...
const handleEmailPayslip = async () => {
    ...
    try {
        ...
        toast.error("Email failed", payrollStore.error || error?.response?.data?.message || "Failed to email payslip.");
    } finally {
...
const employeeContactEmail = computed(() => payslip.value?.employee_email || authStore.user?.email);
...
const isAdjustmentExpanded = (adjustmentId) => expandedAdjustmentIds.value.includes(adjustmentId);
                                ...
                                <div class="text-right">
                                    <span
                                        :class="[
                                            ...
                                            Number(adjustment.amount_delta || 0) >= 0
                                                ? 'text-green-700'
                                                : 'text-red-700',
                                        ]"
                                    >
                                    ...
                                    >
                                        <span>
                                            {{ isAdjustmentExpanded(adjustment.id) ? "Hide details" : "View details" }}
                                        </span>
