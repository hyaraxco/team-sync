...
const totalHours = computed(() =>
  myRecords.value.reduce((sum, record) => sum + Number(record.hours || 0), 0),
);
...
const pendingCount = computed(
  () => myRecords.value.filter((record) => record.status === "pending").length,
);

const rejectedCount = computed(
  () => myRecords.value.filter((record) => record.status === "rejected").length,
);
...
<template>
  <div class="space-y-6">
    <div class="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
      <div>
        <p class="text-xs font-semibold uppercase tracking-[0.14em] text-[#0C51D9]">
          Self Service
        </p>
    ...
    <div class="rounded-[20px] border border-[#DCDEDD] bg-white p-4 sm:p-5">
      <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p class="text-sm text-gray-500">
            Showing {{ meta.from || 0 }} - {{ meta.to || myRecords.length }} of {{ meta.total || 0 }} records
          </p>
        </div>
      ...
      <div v-else-if="!error && myRecords.length === 0" class="mt-5">
        <EmptyState
          ...
          :subtitle="hasActiveFilter ? 'No records match the selected status.' : 'Your approved or pending overtime history will appear here.'"
        />
      </div>
        ...
        >
          <div class="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
            ...
            <div class="rounded-[14px] bg-[#F8FAFC] px-4 py-3 text-left md:text-right">
              <p class="text-xs font-semibold uppercase tracking-[0.1em] text-gray-500">
                Hours
              </p>
