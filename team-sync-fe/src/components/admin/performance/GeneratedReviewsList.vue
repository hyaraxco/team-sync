...
const generateReviews = async () => {
  if (!confirm('Are you sure you want to generate reviews based on the assignment rules? This will only generate reviews for staff members who do not have one yet in this cycle.')) {
    return;
  }
...
<template>
  <MainCard class="mt-6">
    ...
    <!-- Empty State -->
    <div v-if="!props.cycle.reviews?.length" class="text-center py-10 border-2 border-dashed border-gray-200 rounded-xl">
      ...
      <p class="text-sm text-brand-light mt-1 max-w-sm mx-auto">
        Click the "Generate Reviews" button to automatically create review records and assign reviewers based on the system rules.
      </p>
    </div>
      ...
      <table class="w-full">
        <thead>
          <tr class="border-b border-gray-200">
            <th class="text-left py-3 px-4 text-xs font-semibold text-brand-light uppercase tracking-wider">Staff Member</th>
            <th class="text-left py-3 px-4 text-xs font-semibold text-brand-light uppercase tracking-wider">Assigned Reviewer</th>
            <th class="text-left py-3 px-4 text-xs font-semibold text-brand-light uppercase tracking-wider">Status</th>
            <th class="text-right py-3 px-4 text-xs font-semibold text-brand-light uppercase tracking-wider">Action</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="review in props.cycle.reviews" :key="review.id" class="border-b border-gray-100 hover:bg-gray-50">
            <td class="py-3 px-4">
              ...
              <div v-if="review.reviewer" class="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p class="text-xs text-brand-light">
                    ...
                    <span class="inline-block px-2 py-0.5 mt-0.5 rounded text-[10px] bg-blue-100 text-blue-700 border border-blue-200">
            ...
            <td class="py-3 px-4 text-right">
              <button 
                ...
                :class="{ 'opacity-50 cursor-not-allowed': ['completed', 'pending_calibration'].includes(review.status) }"
