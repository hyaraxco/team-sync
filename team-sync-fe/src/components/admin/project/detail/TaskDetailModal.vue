...
const isOwnAssignedTask = computed(() => !!props.task && currentEmployeeId.value === props.task.assignee_id);

const isProjectLeader = computed(() => !!props.task && currentEmployeeId.value === props.task?.project?.leader?.id);

const normalizedTaskStatus = computed(() => {
    return props.task?.status === "pending" ? "todo" : props.task?.status;
});

const isReviewPhaseLocked = computed(() => ["review", "done"].includes(normalizedTaskStatus.value));
...
const canReviewTask = computed(() => can("project-edit") || isProjectLeader.value);
...
const canStartTask = computed(() => can("task-edit") && isOwnAssignedTask.value && isTodoLikeStatus.value);

const canSubmitForReview = computed(
    () => can("task-edit") && isOwnAssignedTask.value && props.task?.status === "in_progress",
);

const canStartRework = computed(() => can("task-edit") && isOwnAssignedTask.value && props.task?.status === "rejected");

const canApproveReview = computed(() => canReviewTask.value && props.task?.status === "review");

const canRejectReview = computed(() => canReviewTask.value && props.task?.status === "review");

const canReopenDoneAsRejected = computed(() => canReviewTask.value && props.task?.status === "done");

const isEmployeeEditableState = computed(() => ["in_progress", "rejected"].includes(normalizedTaskStatus.value));

const isTaskTerminal = computed(() => ["done", "cancelled"].includes(normalizedTaskStatus.value));
...
const canCollaborateTask = computed(() => {
    ...
    if (can("project-edit") || isProjectLeader.value) {
...
const loadTaskCollaborationData = async () => {
    await Promise.all([loadTaskComments(), loadTaskAttachments(), loadTaskStatusLogs()]);
};
...
<template>
    <ModalWrapper :show="isOpen && !!task" maxWidth="4xl" @close="closeModal">
        <!-- Modal Header -->
        <template #header>
        ...
        <div class="py-2">
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-2 space-y-6">
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-2">
                            <StatusBadge type="priority" :value="task.priority" class="!px-3 !py-1.5 text-sm" />
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="bg-gray-50 rounded-lg p-4 text-sm text-gray-600 leading-relaxed min-h-[100px]">
                    ...
                    <div>
                        <div class="flex items-center gap-2 mb-3">
                            ...
                            <h3 class="text-sm font-semibold text-gray-700">Activity</h3>
                        </div>
                        <div class="space-y-3">
                            <div v-if="commentsLoading" class="text-sm text-gray-500 bg-gray-50 rounded-lg px-3 py-2">
                                Loading comments...
                            </div>
                                ...
                                <div class="mt-2">
                                    <template v-if="editingCommentId === comment.id">
                                        <textarea
                                            ...
                                            rows="3"
                                            class="w-full border border-gray-200 rounded-lg p-3 text-sm resize-none focus:border-brand-primary focus:ring-2 focus:ring-brand-primary focus:ring-opacity-20 transition-all"
                                        ></textarea>
                            ...
                            <div class="flex gap-3">
                                ...
                                <div class="flex-1">
                                    <textarea
                                        v-model="newComment"
                                        class="w-full border border-gray-200 rounded-lg p-3 text-sm resize-none focus:border-brand-primary focus:ring-2 focus:ring-brand-primary focus:ring-opacity-20 transition-all disabled:bg-gray-100 disabled:cursor-not-allowed"
                                        :placeholder="
                                            canCollaborateTask
                                                ? 'Write a comment...'
                    ...
                    <div>
                        ...
                        <input ref="fileInputRef" type="file" class="hidden" @change="handleAttachmentSelected" />
                        <button
                            type="button"
                            ...
                            >
                                <div class="min-w-0">
                                    <a
                                        ...
                                        target="_blank"
                                        rel="noreferrer"
                                        class="text-sm font-medium text-brand-primary hover:underline break-all"
                                    >
                ...
                <div class="space-y-6">
                    <!-- Assignee -->
                    <div class="relative">
                        <h3 class="text-xs font-semibold text-gray-500 uppercase mb-3">Assignee</h3>
                        ...
                        <button
                            v-if="canManageAssignee"
                            type="button"
                            class="w-full border border-gray-200 rounded-lg hover:border-brand-primary hover:bg-gray-50 transition-all duration-300 px-3 py-2 flex items-center gap-3 text-left"
                            ...
                            <div class="p-3 border-b border-gray-200">
                                <div class="relative flex items-center">
                                    ...
                                    <input
                                        ...
                                        class="w-full pl-10 pr-3 py-2 text-sm border border-gray-200 rounded-lg focus:border-brand-primary focus:outline-none"
                                    />
                                </div>
                            </div>
                    ...
                    <div>
                        <div class="flex items-center justify-between mb-3">
                            <h3 class="text-xs font-semibold text-gray-500 uppercase">Due Date</h3>
                            <button
                        ...
                        <div v-else class="space-y-2">
                            <input
                                ...
                                class="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:border-brand-primary focus:outline-none"
                            />
                    ...
                    <!-- Status -->
                    <div>
                        <h3 class="text-xs font-semibold text-gray-500 uppercase mb-3">Status</h3>
                        <div :class="getTaskStatusClass(task.status)" class="px-3 py-2 rounded-lg text-sm font-medium">
                    ...
                    <!-- Status Timeline -->
                    <div>
                        <h3 class="text-xs font-semibold text-gray-500 uppercase mb-3">Status Timeline</h3>
                        <div
                    ...
                    <!-- Actions -->
                    <div>
                        <h3 class="text-xs font-semibold text-gray-500 uppercase mb-3">Actions</h3>
                        <div class="space-y-2">
                            <button
                                v-if="canStartTask"
