...
const isOwnAssignedTask = (task) =>
  !!task && currentEmployeeId.value === task.assignee_id;

const isProjectLeader = (task) =>
  !!task && currentEmployeeId.value === task?.project?.leader?.id;
...
const handleColumnDrop = async (value, targetStatus) => {
  const currentTasksInColumn = tasks.value.filter(
    (task) => normalizeStatus(task.status) === targetStatus
  );

  const movedTask = value.find(
    (task) => !currentTasksInColumn.some((columnTask) => columnTask.id === task.id)
  );
