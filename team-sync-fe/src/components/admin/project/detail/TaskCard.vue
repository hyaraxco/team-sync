...
<template>
    ...
    >
        <!-- Task Title -->
        <h5 class="text-sm font-semibold text-gray-900 mb-3 leading-snug group-hover:text-[#0C51D9] transition-colors">
            {{ task.name }}
        </h5>
        ...
        <div class="flex items-center justify-between pt-3 border-t border-gray-100">
            ...
            <div class="flex items-center gap-1.5 px-2 py-1 bg-gray-50 rounded-md">
                ...
                <span class="text-xs font-medium text-gray-600">{{ formatDate(task.due_date) }}</span>
            </div>
        </div>
    </div>
</template>
