...
const taxBpjsOptions = computed(() => ({
  chart: { type: 'area', height: 300, stacked: true, toolbar: { show: false }, fontFamily: 'Plus Jakarta Sans, sans-serif' },
  stroke: { width: 2, curve: 'smooth' },
...
const deductionDonutOptions = computed(() => {
  ...
  return {
    ...
    plotOptions: { pie: { donut: { size: '60%', labels: { show: true, total: { show: true, label: 'Total', fontSize: '14px', formatter: w => formatRupiahCompact(w.globals.seriesTotals.reduce((a, b) => a + b, 0)) } } } } },
    tooltip: { y: { formatter: v => formatRupiah(v) } },
    dataLabels: { enabled: false },
  }
})

const deductionDonutSeries = computed(() =>
  (payroll.value?.deduction_breakdown || []).map(d => d.amount)
)
</script>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      <div class="lg:col-span-2 bg-white border border-[#DCDEDD] rounded-[20px] p-6">
        ...
        <VueApexCharts
          ...
          :options="costTrendOptions" :series="costTrendSeries"
        />
      ...
      <div class="bg-white border border-[#DCDEDD] rounded-[20px] p-6">
        ...
        <VueApexCharts
          ...
          type="bar" height="300"
          :options="salaryDistOptions" :series="salaryDistSeries"
        />
        <div v-else class="flex items-center justify-center h-64 text-gray-400 text-sm">No data</div>
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <div class="bg-white border border-[#DCDEDD] rounded-[20px] p-6">
        ...
        <VueApexCharts
          ...
          type="area" height="300"
          :options="taxBpjsOptions" :series="taxBpjsSeries"
        />
        <div v-else class="flex items-center justify-center h-64 text-gray-400 text-sm">No data</div>
      </div>
      ...
      <div class="bg-white border border-[#DCDEDD] rounded-[20px] p-6">
        ...
        <VueApexCharts
          ...
          type="bar" height="300"
          :options="costByDeptOptions" :series="costByDeptSeries"
        />
        <div v-else class="flex items-center justify-center h-64 text-gray-400 text-sm">No data</div>
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      <div class="bg-white border border-[#DCDEDD] rounded-[20px] p-6">
        ...
        <VueApexCharts
          ...
          :options="deductionDonutOptions" :series="deductionDonutSeries"
        />
        <div v-else class="flex items-center justify-center h-64 text-gray-400 text-sm">No data</div>
      </div>
    </div>
