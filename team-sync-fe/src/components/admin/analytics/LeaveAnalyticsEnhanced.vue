        ...
        >
          ...
          <div class="space-y-2">
            ...
            <div class="text-xs text-gray-500 text-center">
              {{ trend.utilization_rate }}% utilized
            </div>
          </div>
        </div>
      </div>
...
const peakPeriodsChartOptions = computed(() => ({
  ...
  colors: ["#3b82f6"],
  xaxis: {
    categories: (peakLeavePeriods.value?.peak_periods || []).map(
      (p) => p.month,
    ),
  },
...
const leaveTypeDonutSeries = computed(() =>
  (leave.value?.leave_type_distribution || []).map((d) => d.count),
);
...
const statusDonutSeries = computed(() =>
  (leave.value?.status_distribution || []).map((d) => d.count),
);
</script>
