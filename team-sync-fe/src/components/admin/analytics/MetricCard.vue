  ...
  >
    <div class="relative z-10 flex flex-col justify-between h-full">
      ...
      <div v-if="!highlight" class="flex items-center justify-between mb-4">
        ...
        <div
          v-if="icon"
          class="p-2.5 bg-[#F4F7FF] rounded-[10px] group-hover:bg-[#0C51D9] transition-colors"
        >
          <component
            :is="icon"
            class="w-5 h-5 text-[#0C51D9] group-hover:text-white transition-colors"
          />
        </div>
      </div>
        ...
        <div class="flex items-center justify-between relative">
          <div :class="highlight ? 'flex flex-col space-y-1' : 'w-full'">
            <div
              ...
              :class="!highlight ? 'justify-between' : ''"
            >
              <span
          ...
          <div
            v-if="highlight && icon"
            class="absolute right-0 top-1/2 -translate-y-1/2 -mr-1"
          >
            <div
              class="p-5 bg-slate-500/20 rounded-[26px] backdrop-blur-xl shrink-0"
            >
    ...
    >
      <div
        class="w-full max-w-[60%] h-2 bg-slate-200/50 rounded-full overflow-hidden"
      >
        <div
          class="h-full bg-brand-primary w-1/2 animate-[progress_1s_ease-in-out_infinite]"
        ></div>
      </div>
    </div>
  </div>
