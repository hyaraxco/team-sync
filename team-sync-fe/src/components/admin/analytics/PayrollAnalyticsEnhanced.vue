...
const { payroll, payrollLoading, payrollCostTrends, salaryDistribution, deductionAnalysis, enhancedMetricsLoading } =
...
const costTrendOptions = computed(() => ({
    ...
    xaxis: {
        categories: (payrollCostTrends.value?.total_cost_trend || []).map((d) => d.period),
    },
    yaxis: {
        labels: {
...
const salaryDistChartOptions = computed(() => ({
    ...
    colors: ["#3b82f6"],
    xaxis: {
        categories: (salaryDistribution.value?.distribution || []).map((d) => d.range),
    },
...
const departmentCostSeries = computed(() => (payroll.value?.department_costs || []).map((d) => d.cost));
</script>
