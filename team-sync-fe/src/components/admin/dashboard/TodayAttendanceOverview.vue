...
const checkedInPercentage = computed(() => {
  ...
  return Math.round(
    (todayAttendance.value.checked_in_count / todayAttendance.value.total_employees) * 100
  );
});
  ...
  <div class="bg-white border border-[#DCDEDD] rounded-[20px] p-4 sm:p-5">
    <div class="flex items-center justify-between mb-4">
      <div class="flex items-center gap-3">
        <div class="w-10 h-10 bg-gradient-to-br from-[#0C51D9] to-[#3B82F6] rounded-[12px] flex items-center justify-center">
    ...
    <template v-else-if="todayAttendance">
      ...
      <div class="mb-4">
        <div class="flex items-center justify-between text-xs text-brand-light mb-1.5">
          <span>{{ todayAttendance.checked_in_count }} / {{ todayAttendance.total_employees }} employees</span>
