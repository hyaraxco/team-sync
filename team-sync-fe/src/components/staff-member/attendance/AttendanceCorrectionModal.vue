...
const handleSubmit = () => {
    ...
    const payload = {
        ...
        requested_check_in: form.value.requested_check_in ? new Date(form.value.requested_check_in).toISOString() : null,
        requested_check_out: form.value.requested_check_out ? new Date(form.value.requested_check_out).toISOString() : null,
...
<template>
  <div
    v-if="show"
    class="fixed inset-0 z-50 flex items-center justify-center bg-brand-dark/50 p-4"
  >
    <div
      class="bg-white rounded-[24px] w-full max-w-lg overflow-hidden shadow-2xl"
    >
      ...
      <div class="p-6">
        ...
        <div class="space-y-4">
            <div>
                 ...
                 <input v-model="form.requested_check_in" type="datetime-local" class="w-full px-4 py-2 border rounded-[8px]" />
            </div>

            <div>
                 <label class="block text-sm font-semibold text-brand-dark mb-1">Check Out Time</label>
                 <input v-model="form.requested_check_out" type="datetime-local" class="w-full px-4 py-2 border rounded-[8px]" />
            </div>

            <div>
                 <label class="block text-sm font-semibold text-brand-dark mb-1">Reason</label>
                 <textarea v-model="form.reason" class="w-full px-4 py-2 border rounded-[8px]" rows="3" placeholder="Why are you making this correction?" required></textarea>
            </div>
        </div>
