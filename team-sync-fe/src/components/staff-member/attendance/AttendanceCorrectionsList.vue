    ...
    <div class="bg-white border flex-1 border-[#DCDEDD] rounded-[20px] p-6 mb-6">
        ...
        <div v-if="!corrections || corrections.length === 0">
           <EmptyState
              ...
              description="You have not made any attendance correction requests."
            />
        </div>
        <div v-else class="space-y-4">
             ...
             >
                 <div>
                     <p class="font-bold text-brand-dark">{{ correction.attendance ? formatDateShort(correction.attendance.date) : 'Unknown Date' }}</p>
                     ...
                     <div class="flex gap-4 mt-3">
                         <div class="text-xs bg-gray-50 px-2 py-1 rounded">
                             ...
                             <span class="text-brand-dark">{{ correction.requested_check_in ? formatTime(correction.requested_check_in) : '-' }}</span>
                         </div>
                         <div class="text-xs bg-gray-50 px-2 py-1 rounded">
                             <span class="font-semibold text-gray-500 block">Requested Out</span>
                             <span class="text-brand-dark">{{ correction.requested_check_out ? formatTime(correction.requested_check_out) : '-' }}</span>
                         </div>
                     </div>
                 </div>

                 <div class="flex flex-col items-end justify-between">
                     <span :class="['px-3 py-1 rounded-full text-xs font-semibold capitalize', getStatusBadge(correction.status)]">
                         {{ correction.status }}
                     </span>
