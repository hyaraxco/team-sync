...
describe("Payroll Store - Phase 3 Features", () => {
    ...
    describe("#4 - Settings Version Diff", () => {
        it("fetchSettingVersionDiff calls correct endpoint", async () => {
            const mockDiff = {
                ...
                version_number: 3,
                has_previous: true,
                changes: [{ field: "payday_day", old_value: 25, new_value: 28 }],
            };
    ...
    describe("#6 - Approval Matrix", () => {
        it("fetchApprovalPolicies calls correct endpoint", async () => {
            const mockPolicies = [{ id: 1, name: "Director Approval", min_amount: 1000000 }];
            axiosInstance.get.mockResolvedValueOnce({
    ...
    describe("#7 - Enhanced Analytics", () => {
        it("fetchPayrollAnalytics returns enhanced metrics", async () => {
            const mockAnalytics = {
                ...
                average_salary_trend: [{ salary_month: "2026-03-01", label: "Mar 2026", average_salary: 10000000 }],
                total_deductions_trend: [{ salary_month: "2026-03-01", label: "Mar 2026", total_deductions: 500000 }],
